...
//PUT-запрос по ссылке /updateHotel для обновления отеля
router.put(
  "/updateHotel",
  //Выполнение асинхронной функции
  async (req, res) => {
    ...
    try {
      //Получаем значения отправленных полей
      const { _id, name, description, id_manager } = req.body;
      ...
      console.log("\nОшибка API-сервера в users.routes router.put('/updateHotel').\n", e);
      res.status(400).send({ message: "Ошибка API-сервера в users.routes router.put('/updateHotel')." });
    }
  }
);
